...
services:
  traefik:
    ...
    labels:
      - "traefik.enable=true"
      - "traefik.http.routers.traefik.rule=Host(`${HOST_NAME}`) && (PathPrefix(`/traefik`) || PathPrefix(`/api`))"
  ...
  api:
    labels:
      - "traefik.enable=true"
      - "traefik.http.routers.api.rule=Host(`${HOST_NAME}`) && PathPrefix(`/app/cicd/api`)"
